...
| Дашборд | Панель живых отчётов |
|---|---|
| ![Дашборд](docs/dashboard.png) | ![Отчёты](docs/reports.png) |

| Вид сверху — тема Terraria | Тема Doom (игроки — демоны) |
|---|---|
| ![Карта Terraria](docs/map-terraria.png) | ![Карта Doom](docs/map-doom.png) |

| Тесты API | Мастер первого запуска |
|---|---|
| ![Тесты API](docs/api-tests.png) | ![Мастер](docs/wizard.png) |
...
- **🔌 Контроль связи** — перед стартом и дальше каждые 20 с проверяются оба канала до клуба: Gizmo API и SQL. Нет связи на старте — симуляция не начинается, а ждёт и пишет, что именно лежит и через сколько следующая проверка (пауза растёт 5→60 с). Связь оборвалась в процессе — мир замораживается (никаких «⚠» в ленту) и сам оживает, когда клуб ответит. Видно и в консоли, и в вебе: мигающая лампочка «Связь» в шапке плюс экран ожидания с обратным отсчётом и кнопкой «Проверить сейчас».
